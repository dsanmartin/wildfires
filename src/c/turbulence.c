...
void turbulence_v1_debug(double *R_turbulence, double *R_new, Parameters *parameters) {
    ...
    double fw, fwx, fwy, fwz;
    int im1, ip1, jm1, jp1;
    // Pointers to debug sgs model
    double *sgs_x_debug = (double *)malloc(Nx * Ny * Nz * sizeof(double));
    double *sgs_y_debug = (double *)malloc(Nx * Ny * Nz * sizeof(double));
    double *sgs_z_debug = (double *)malloc(Nx * Ny * Nz * sizeof(double));
    double *sgs_q_debug = (double *)malloc(Nx * Ny * Nz * sizeof(double));
    // Files to write debug information
    FILE *sgs_x_file = fopen("data/output/debug/sgs_x_debug.bin", "wb");
    FILE *sgs_y_file = fopen("data/output/debug/sgs_y_debug.bin", "wb");
    FILE *sgs_z_file = fopen("data/output/debug/sgs_z_debug.bin", "wb");
    FILE *sgs_q_file = fopen("data/output/debug/sgs_q_debug.bin", "wb");
    // Loop over nodes
    for (int i = 0; i < Nx; i++) {
        for (int j = 0; j < Ny; j++) {
            for (int k = 1; k < Nz - 1; k++) {
                ...
                sgs_x_debug[IDX(i, j, k, Nx, Ny, Nz)] = sgs_x;
                sgs_y_debug[IDX(i, j, k, Nx, Ny, Nz)] = sgs_y;
                sgs_z_debug[IDX(i, j, k, Nx, Ny, Nz)] = sgs_z;
                sgs_q_debug[IDX(i, j, k, Nx, Ny, Nz)] = sgs_q;
            }
        }
    }
    // Write debug information
    fwrite(sgs_x_debug, sizeof(double), Nx * Ny * Nz, sgs_x_file);
    fwrite(sgs_y_debug, sizeof(double), Nx * Ny * Nz, sgs_y_file);
    fwrite(sgs_z_debug, sizeof(double), Nx * Ny * Nz, sgs_z_file);
    fwrite(sgs_q_debug, sizeof(double), Nx * Ny * Nz, sgs_q_file);
    // Close files
    fclose(sgs_x_file);
    fclose(sgs_y_file);
    fclose(sgs_z_file);
