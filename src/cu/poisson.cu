...
void thomas_algorithm(cufftDoubleComplex *a, cufftDoubleComplex *b, cufftDoubleComplex *c, cufftDoubleComplex *d, cufftDoubleComplex *x, cufftDoubleComplex *l, cufftDoubleComplex *u, cufftDoubleComplex *y, Parameters parameters) {
    ...
    for (int ij = idx; ij < size; ij += stride) {
        int i = ij / (Ny - 1);
        int j = ij % (Ny - 1);
        u[IDX(i, j, 0, Nx - 1, Ny - 1, Nz - 1)] = b[IDX(i, j, 0, Nx - 1, Ny - 1, Nz - 1)];
        for (int k = 1; k < Nz - 1; k++) {
            l[IDX(i, j, k - 1, Nx - 1, Ny - 1, Nz - 2)] = cuCdiv(a[IDX(i, j, k - 1, Nx - 1, Ny - 1, Nz - 2)], u[IDX(i, j, k - 1, Nx - 1, Ny - 1, Nz - 1)]); 
            u[IDX(i, j, k, Nx - 1, Ny - 1, Nz - 1)] = cuCsub(b[IDX(i, j, k, Nx - 1, Ny - 1, Nz - 1)], cuCmul(l[IDX(i, j, k - 1, Nx - 1, Ny - 1, Nz - 2)], c[IDX(i, j, k - 1, Nx - 1, Ny - 1, Nz - 2)]));
        }
...
__global__
void thomas_algorithm_v1(cufftDoubleComplex *a, cufftDoubleComplex *b, cufftDoubleComplex *c, cufftDoubleComplex *d, cufftDoubleComplex *x, cufftDoubleComplex *l, cufftDoubleComplex *u, cufftDoubleComplex *y, int N) {
    int i = threadIdx.x;
    if (i == 0) {
        // Create L and U
        u[0] = b[0];
...
__global__
void copy_to_p_in_2(cufftDoubleComplex *p_in, cufftDoubleComplex *pk, Parameters parameters) {
    int Nx = parameters.Nx;
    int Ny = parameters.Ny;
    int Nz = parameters.Nz;
    int idx = threadIdx.x;
    ...
    for (int rsk = idx; rsk < size; rsk += stride) {
        ...
        int k = rsk % (Nz - 1);
        p_in[FFTWIDX(r, s, k, Nx - 1, Ny - 1, Nz - 1)] = pk[IDX(r, s, k, Nx - 1, Ny - 1, Nz - 1)];
    }
}
