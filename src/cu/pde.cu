...
void Phi(double t, double *R_old, double *R_new, double *R_turbulence, Parameters *parameters) {
    RHS<<<parameters->number_of_blocks, parameters->threads_per_block>>>(t, R_old, R_new, R_turbulence, parameters);
    cudaDeviceSynchronize();
    turbulence<<<parameters->number_of_blocks, parameters->threads_per_block>>>(R_turbulence, R_new, parameters);
    cudaDeviceSynchronize();
    boundary_conditions<<<parameters->number_of_blocks, parameters->threads_per_block>>>(R_new, parameters);
    cudaDeviceSynchronize();
}
