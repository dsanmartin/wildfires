...
__global__
void RHS_old(double t, double *R_old, double *R_new, double *R_turbulence, Parameters parameters) {
    ...
    for (int ijk = idx; ijk < size; ijk += stride) {
        ...
        S = H_R * Y_ijk * K_T * H_step / c_p - h * a_v * (T_ijk - T_inf) / (c_p * rho);
...
void RHS(double t, double *R_old, double *R_new, double *R_turbulence, Parameters parameters, double *z) {
    ...
    for (int ijk = idx; ijk < size; ijk += stride) {
        ...
        int im2 = (i - 2 + Nx - 1) % (Nx - 1);
        int jm1 = (j - 1 + Ny - 1) % (Ny - 1);
        int jm2 = (j - 2 + Ny - 1) % (Ny - 1);
        int ip1 = (i + 1) % (Nx - 1);
        int ip2 = (i + 2) % (Nx - 1);
        int jp1 = (j + 1) % (Ny - 1);
        int jp2 = (j + 2) % (Ny - 1);
        ...
        // Current nodes \phi_{i,j,k}
        double u_ijk = R_old[u_index + IDX(i, j, k, Nx, Ny, Nz)];
        double v_ijk = R_old[v_index + IDX(i, j, k, Nx, Ny, Nz)];
        double w_ijk = R_old[w_index + IDX(i, j, k, Nx, Ny, Nz)];
        double T_ijk = R_old[T_index + IDX(i, j, k, Nx, Ny, Nz)];
        // \phi_{i-1,j,k}
        double u_im1jk = R_old[u_index + IDX(im1, j, k, Nx, Ny, Nz)];
        double v_im1jk = R_old[v_index + IDX(im1, j, k, Nx, Ny, Nz)];
        double w_im1jk = R_old[w_index + IDX(im1, j, k, Nx, Ny, Nz)];
        double T_im1jk = R_old[T_index + IDX(im1, j, k, Nx, Ny, Nz)];
        // \phi_{i+1,j,k}
        double u_ip1jk = R_old[u_index + IDX(ip1, j, k, Nx, Ny, Nz)];
        double v_ip1jk = R_old[v_index + IDX(ip1, j, k, Nx, Ny, Nz)];
        double w_ip1jk = R_old[w_index + IDX(ip1, j, k, Nx, Ny, Nz)];
        double T_ip1jk = R_old[T_index + IDX(ip1, j, k, Nx, Ny, Nz)];
        // \phi_{i-2,j,k}
        double u_im2jk = R_old[u_index + IDX(im2, j, k, Nx, Ny, Nz)];
        double v_im2jk = R_old[v_index + IDX(im2, j, k, Nx, Ny, Nz)];
        double w_im2jk = R_old[w_index + IDX(im2, j, k, Nx, Ny, Nz)];
        // \phi_{i+2,j,k}
        double u_ip2jk = R_old[u_index + IDX(ip2, j, k, Nx, Ny, Nz)];
        double v_ip2jk = R_old[v_index + IDX(ip2, j, k, Nx, Ny, Nz)];
        double w_ip2jk = R_old[w_index + IDX(ip2, j, k, Nx, Ny, Nz)];
        // \phi_{i,j-1,k}
        double u_ijm1k = R_old[u_index + IDX(i, jm1, k, Nx, Ny, Nz)];
        double v_ijm1k = R_old[v_index + IDX(i, jm1, k, Nx, Ny, Nz)];
        double w_ijm1k = R_old[w_index + IDX(i, jm1, k, Nx, Ny, Nz)];
        double T_ijm1k = R_old[T_index + IDX(i, jm1, k, Nx, Ny, Nz)];
        // \phi_{i,j+1,k}
        double u_ijp1k = R_old[u_index + IDX(i, jp1, k, Nx, Ny, Nz)];
        double v_ijp1k = R_old[v_index + IDX(i, jp1, k, Nx, Ny, Nz)];
        double w_ijp1k = R_old[w_index + IDX(i, jp1, k, Nx, Ny, Nz)];
        double T_ijp1k = R_old[T_index + IDX(i, jp1, k, Nx, Ny, Nz)];
        // \phi_{i,j-2,k}
        double u_ijm2k = R_old[u_index + IDX(i, jm2, k, Nx, Ny, Nz)];
        double v_ijm2k = R_old[v_index + IDX(i, jm2, k, Nx, Ny, Nz)];
        double w_ijm2k = R_old[w_index + IDX(i, jm2, k, Nx, Ny, Nz)];
        // \phi_{i,j+2,k}
        double u_ijp2k = R_old[u_index + IDX(i, jp2, k, Nx, Ny, Nz)];
        double v_ijp2k = R_old[v_index + IDX(i, jp2, k, Nx, Ny, Nz)];
        double w_ijp2k = R_old[w_index + IDX(i, jp2, k, Nx, Ny, Nz)];
        // \phi_{i,j,k-1}
        double u_ijkm1 = 0, v_ijkm1 = 0, w_ijkm1 = 0, T_ijkm1 = 0;
        if (k > 0) {
            u_ijkm1 = R_old[u_index + IDX(i, j, k - 1, Nx, Ny, Nz)];
            v_ijkm1 = R_old[v_index + IDX(i, j, k - 1, Nx, Ny, Nz)];
            w_ijkm1 = R_old[w_index + IDX(i, j, k - 1, Nx, Ny, Nz)];
        ...
        // \phi_{i,j,k+1}
        double u_ijkp1 = 0, v_ijkp1 = 0, w_ijkp1 = 0, T_ijkp1 = 0;
        if (k < Nz - 1) {
            u_ijkp1 = R_old[u_index + IDX(i, j, k + 1, Nx, Ny, Nz)];
            v_ijkp1 = R_old[v_index + IDX(i, j, k + 1, Nx, Ny, Nz)];
            w_ijkp1 = R_old[w_index + IDX(i, j, k + 1, Nx, Ny, Nz)];
        ...
        // \phi_{i,j,k-2}
        double u_ijkm2 = 0, v_ijkm2 = 0, w_ijkm2 = 0, T_ijkm2 = 0;
        if (k > 1) {
            u_ijkm2 = R_old[u_index + IDX(i, j, k - 2, Nx, Ny, Nz)];
            v_ijkm2 = R_old[v_index + IDX(i, j, k - 2, Nx, Ny, Nz)];
            w_ijkm2 = R_old[w_index + IDX(i, j, k - 2, Nx, Ny, Nz)];
        ...
        // \phi_{i,j,k+2}
        double u_ijkp2 = 0, v_ijkp2 = 0, w_ijkp2 = 0, T_ijkp2 = 0;
        if (k < Nz - 2) {
            u_ijkp2 = R_old[u_index + IDX(i, j, k + 2, Nx, Ny, Nz)];
            v_ijkp2 = R_old[v_index + IDX(i, j, k + 2, Nx, Ny, Nz)];
            w_ijkp2 = R_old[w_index + IDX(i, j, k + 2, Nx, Ny, Nz)];
        ...
        // \phi_{i,j,k-3}
        double u_ijkm3 = 0, v_ijkm3 = 0, w_ijkm3 = 0, T_ijkm3 = 0;
        if (k > 2) {
            u_ijkm3 = R_old[u_index + IDX(i, j, k - 3, Nx, Ny, Nz)];
            v_ijkm3 = R_old[v_index + IDX(i, j, k - 3, Nx, Ny, Nz)];
            w_ijkm3 = R_old[w_index + IDX(i, j, k - 3, Nx, Ny, Nz)];
        ...
        // \phi_{i,j,k+3}
        double u_ijkp3 = 0, v_ijkp3 = 0, w_ijkp3 = 0, T_ijkp3 = 0;
        if (k < Nz - 3) {
            u_ijkp3 = R_old[u_index + IDX(i, j, k + 3, Nx, Ny, Nz)];
            v_ijkp3 = R_old[v_index + IDX(i, j, k + 3, Nx, Ny, Nz)];
            w_ijkp3 = R_old[w_index + IDX(i, j, k + 3, Nx, Ny, Nz)];
        ...
        /* Computing upwind scheme terms */
        double u_plu = MAX(u_ijk, 0.0);
        double u_min = MIN(u_ijk, 0.0);
        double v_plu = MAX(v_ijk, 0.0);
        double v_min = MIN(v_ijk, 0.0);
        double w_plu = MAX(w_ijk, 0.0);
        double w_min = MIN(w_ijk, 0.0);
        double u_im = (3 * u_ijk - 4 * u_im1jk + u_im2jk) / (2 * dx);
        double v_im = (3 * v_ijk - 4 * v_im1jk + v_im2jk) / (2 * dx);
        double w_im = (3 * w_ijk - 4 * w_im1jk + w_im2jk) / (2 * dx);
        double u_ip = (-3 * u_ijk + 4 * u_ip1jk - u_ip2jk) / (2 * dx);
        double v_ip = (-3 * v_ijk + 4 * v_ip1jk - v_ip2jk) / (2 * dx);
        double w_ip = (-3 * w_ijk + 4 * w_ip1jk - w_ip2jk) / (2 * dx);
        double u_jm = (3 * u_ijk - 4 * u_ijm1k + u_ijm2k) / (2 * dy);
        double v_jm = (3 * v_ijk - 4 * v_ijm1k + v_ijm2k) / (2 * dy);
        double w_jm = (3 * w_ijk - 4 * w_ijm1k + w_ijm2k) / (2 * dy);
        double u_jp = (-3 * u_ijk + 4 * u_ijp1k - u_ijp2k) / (2 * dy);
        double v_jp = (-3 * v_ijk + 4 * v_ijp1k - v_ijp2k) / (2 * dy);
        double w_jp = (-3 * w_ijk + 4 * w_ijp1k - w_ijp2k) / (2 * dy);
        // Second order forward difference at k=0 and k=1
        double u_km, v_km, w_km;
        if (k <= 1) {
            u_km = (-3 * u_ijk + 4 * u_ijkp1 - u_ijkp2) / (2 * dz);
            v_km = (-3 * v_ijk + 4 * v_ijkp1 - v_ijkp2) / (2 * dz);
            w_km = (-3 * w_ijk + 4 * w_ijkp1 - w_ijkp2) / (2 * dz);
        ...
        /* Compute first partial derivatives */
        // Upwind scheme for velocity
        double uux = u_plu * u_im + u_min * u_ip; // u * du/dx
        double vuy = v_plu * u_jm + v_min * u_jp; // v * du/dy
        double wuz = w_plu * u_km + w_min * u_kp; // w * du/dz
        double uvx = u_plu * v_im + u_min * v_ip; // u * dv/dx
        double vvy = v_plu * v_jm + v_min * v_jp; // v * dv/dy
        double wvz = w_plu * v_km + w_min * v_kp; // w * dv/dz
        double uwx = u_plu * w_im + u_min * w_ip; // u * dw/dx
        double vwy = v_plu * w_jm + v_min * w_jp; // v * dw/dy
        double wwz = w_plu * w_km + w_min * w_kp; // w * dw/dz
        // Central difference for temperature and turbulence velocity
        double ux  = (u_ip1jk - u_im1jk) / (2 * dx); // du/dx
        double uy  = (u_ijp1k - u_ijm1k) / (2 * dy); // du/dy                
        double vx  = (v_ip1jk - v_im1jk) / (2 * dx); // dv/dx
        double vy  = (v_ijp1k - v_ijm1k) / (2 * dy); // dv/dy                
        double wx  = (w_ip1jk - w_im1jk) / (2 * dx); // dw/dx
        double wy  = (w_ijp1k - w_ijm1k) / (2 * dy); // dw/dy                
        double Tx  = (T_ip1jk - T_im1jk) / (2.0 * dx); // dT/dx
        double Ty  = (T_ijp1k - T_ijm1k) / (2.0 * dy); // dT/dy                
        /* Compute second partial derivatives */
        // Central difference for velocity and temperature
        double uxx = (u_ip1jk - 2.0 * u_ijk + u_im1jk) / (dx * dx); // d^2u/dx^2
        double uyy = (u_ijp1k - 2.0 * u_ijk + u_ijm1k) / (dy * dy); // d^2u/dy^2
        double vxx = (v_ip1jk - 2.0 * v_ijk + v_im1jk) / (dx * dx); // d^2v/dx^2
        double vyy = (v_ijp1k - 2.0 * v_ijk + v_ijm1k) / (dy * dy); // d^2v/dy^2
        double wxx = (w_ip1jk - 2.0 * w_ijk + w_im1jk) / (dx * dx); // d^2w/dx^2
        double wyy = (w_ijp1k - 2.0 * w_ijk + w_ijm1k) / (dy * dy); // d^2w/dy^2
        double Txx = (T_ip1jk - 2.0 * T_ijk + T_im1jk) / (dx * dx); // d^2T/dx^2 
        double Tyy = (T_ijp1k - 2.0 * T_ijk + T_ijm1k) / (dy * dy); // d^2T/dy^2
        double uz, vz, wz, Tz, uzz, vzz, wzz, Tzz;
        if (k == 0) { // Forward difference
            uz = (-3 * u_ijk + 4 * u_ijkp1 - u_ijkp2) / (2 * dz); // du/dz
            vz = (-3 * v_ijk + 4 * v_ijkp1 - v_ijkp2) / (2 * dz); // dv/dz
            wz = (-3 * w_ijk + 4 * w_ijkp1 - w_ijkp2) / (2 * dz); // dw/dz
        ...
        R_turbulence[parameters.turbulence_indexes.fw  + IDX(i, j, k, Nx, Ny, Nz)] = fw;
        /* Compute fuel and source term */
        double H_step = (T_ijk > T_pc) ? 1.0 : 0.0;
        double K_T = A * exp(-T_a / T_ijk);
        double Y_ijk = 0.0;
        double Y_RHS;
        if (k < Nz_Y) {
            Y_ijk = R_old[Y_index + IDX(i, j, k, Nx, Ny, Nz_Y)];
        ...
        // Compute source and force terms
        // S = source(T_ijk, Y_ijk, H_R, A, T_a, h, a_v, T_inf, c_p, rho, T_pc);
        double S = H_R * Y_ijk * K_T * H_step / c_p - h * a_v * (T_ijk - T_inf) / (c_p * rho);
        double mod_U = sqrt(u_ijk * u_ijk + v_ijk * v_ijk + w_ijk * w_ijk);
        // Force terms
        double F_x = - Y_D * a_v * Y_ijk * mod_U * u_ijk;
        double F_y = - Y_D * a_v * Y_ijk * mod_U * v_ijk;                             
        double F_z = - g * (T_ijk - T_inf) / T_ijk - Y_D * a_v * Y_ijk * mod_U * w_ijk;
        // Compute Laplacian terms
        double lap_u = uxx + uyy + uzz;
        double lap_v = vxx + vyy + vzz;
        double lap_w = wxx + wyy + wzz;
        double lap_T = Txx + Tyy + Tzz;
        // Compute RHS
        double u_RHS = nu * lap_u - (uux + vuy + wuz) + F_x;
        double v_RHS = nu * lap_v - (uvx + vvy + wvz) + F_y;
        double w_RHS = nu * lap_w - (uwx + vwy + wwz) + F_z;
        double T_RHS = alpha * lap_T - (u_ijk * Tx + v_ijk * Ty + w_ijk * Tz) + S;
        // Save RHS into R_new
        R_new[u_index + IDX(i, j, k, Nx, Ny, Nz)] = u_RHS;
        R_new[v_index + IDX(i, j, k, Nx, Ny, Nz)] = v_RHS;
        R_new[w_index + IDX(i, j, k, Nx, Ny, Nz)] = w_RHS;
...
void Phi(double t, double *R_old, double *R_new, double *R_turbulence, Parameters parameters, double *z) {
    RHS<<<BLOCKS, THREADS>>>(t, R_old, R_new, R_turbulence, parameters, z);
    checkCuda(cudaGetLastError());
    checkCuda(cudaDeviceSynchronize());
    turbulence<<<BLOCKS, THREADS>>>(R_turbulence, R_new, parameters);
    checkCuda(cudaGetLastError());
    checkCuda(cudaDeviceSynchronize());
    boundary_conditions<<<BLOCKS, THREADS>>>(R_new, parameters);
    checkCuda(cudaGetLastError());
    checkCuda(cudaDeviceSynchronize());
}
