    ...
    for (int i = 1; i < Nx - 1; i++) {
        for (int j = 1; j < Ny - 1; j++) {
            for (int k = 1; k < Nz - 1; k++) {
                ...
                vx = v_plu * v_im + v_min * v_ip; // dv/dx
                vy = v_plu * v_jm + v_min * v_jp; // dv/dy
                vz = v_plu * v_km + v_min * v_kp; // dv/dz
                wx = w_plu * w_im + w_min * w_ip; // dw/dx
                wy = w_plu * w_jm + w_min * w_jp; // dw/dy
                wz = w_plu * w_km + w_min * w_kp; // dw/dz
