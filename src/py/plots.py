...
from showc import load_c
sys.path.append('/home/dsanmartin/wf/src')
from derivatives import compute_gradient_plots, compute_curl_plots, compute_divergence_plots
...
def load_data_for_plots(data_path: str, parameters_path: str, plots: list, tn: int = None) -> tuple:
    ...
    topo = False
    if topo:
        c_vars, domain, topography = load_c(data_path, topo=topo)
    else:
        c_vars, domain = load_c(data_path, topo=topo)
    x, y, z, t = domain
    ...
    if 'Y' in plots:
        ...
        terrain[:] = np.nan
        if topo:
            _, _, Z = np.meshgrid(x, y, z)
            topography = np.repeat(topography[:, :, np.newaxis], z.shape[0], axis=2)            
            terrain[Z < topography] = 1
        # terrain[Y[0] == 1] = 1
        data_plots['Y'] = {
            'data': [Y, terrain],
...
def plot_vector_field(ax: plt.Axes, x: np.ndarray, y: np.ndarray, u: np.ndarray, v: np.ndarray, 
    ...
    None
    """
    X, Y, U, V = x[::qs], y[::qs], u[::qs,::qs], v[::qs,::qs]
    if streamplot: 
        ...
        ax.quiver(X, Y, U, V, angles='xy', scale_units='xy', color=color)
    return None
...
def plot_2D(n: int, domain: tuple, plots: dict, plot_lims: list, visualization: str = 'vertical',
        ...
        density: float = 1, bounds: bool = True, ticks: bool = True, x_i: int = 0, y_j: int = 0, z_k: int = 0, figsize: tuple = None) -> None:
    """
